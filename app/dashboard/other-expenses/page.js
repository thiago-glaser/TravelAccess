            ...
            <div className="max-w-3xl mx-auto">
                <header className="mb-12 flex justify-between items-end">
                    <div>
                        ...
                        <p className="text-gray-500 mt-2">{t('expenses.subtitle') || 'Manage trackable expenses for your vehicles'}</p>
                    </div>
                </header>
                ...
                <div className="grid gap-8">
                    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-xl">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <Link href="/dashboard/expense-types" className="text-xs bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg border border-blue-200 font-bold hover:bg-blue-100 transition-colors">
                        ...
                        <form onSubmit={handleAddExpense} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="flex flex-col gap-1">
                                <label className="text-sm text-gray-500 font-medium">{t('expenses.carLabel') || 'Car'}</label>
                                <select
                                    value={selectedCarId}
                            ...
                            <div className="flex flex-col gap-1">
                                <label className="text-sm text-gray-500 font-medium">{t('expenses.typeLabel') || 'Expense Type'}</label>
                                <select
                            ...
                            <div className="flex flex-col gap-1">
                                <label className="text-sm text-gray-500 font-medium">{t('expenses.dateTimeLabel') || 'Date & Time'}</label>
                                <input
                                    type="datetime-local"
                                    step="1"
                                    value={expenseDate}
                            ...
                            <div className="flex flex-col gap-1">
                                <label className="text-sm text-gray-500 font-medium">{t('expenses.amountLabel') || 'Amount'}</label>
                            ...
                            <div className="flex flex-col gap-1 md:col-span-2">
                                <label className="text-sm text-gray-500 font-medium">{t('expenses.descriptionLabel') || 'Description'}</label>
                                <textarea
                                    value={description}
                                    onChange={(e) => setDescription(e.target.value)}
                                    placeholder={t('expenses.descriptionPlaceholder') || 'Enter details...'}
                                    maxLength="1000"
                                    className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all placeholder-gray-400 min-h-[100px]"
                                    required
                                />
                            ...
                            <div className="md:col-span-2 flex flex-col gap-2 mt-2">
                                <label className="text-sm text-gray-500 font-medium">{t('expenses.receiptLabel') || 'Receipt (Optional)'}</label>
                                <div className="flex flex-col items-start gap-4 p-4 border border-dashed border-gray-300 rounded-xl bg-gray-50">
                                    <input
                                        ...
                                        className="text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition-colors file:cursor-pointer"
                                    />
                                    {receiptPreview && (
                                        <div className="relative group">
                                            {receiptPreview === 'pdf' ? (
                                                ...
                                                <img src={receiptPreview} alt="Receipt Preview" className="h-32 rounded-lg border border-gray-200 object-contain bg-gray-100" />
                                            )}
                                            <button
                                                type="button"
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-xl overflow-hidden">
                        <div className="p-6 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                            ...
                            <span className="text-xs bg-gray-200 text-gray-600 px-3 py-1 rounded-full">{expenses.length} total</span>
                        </div>
                        ...
                        <div className="divide-y divide-gray-100">
                            {loading ? (
                                ...
                                <div className="p-10 text-center text-gray-500 font-medium italic">No expenses added yet.</div>
                            ) : (
                                expenses.map((entry) => {
                                            ...
                                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                                                <div className="flex-1">
                                                    <div className="flex items-center gap-3 mb-2">
                                                        <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center border border-blue-200 flex-shrink-0 text-blue-600 font-bold">$</div>
                                                        <div>
                                                    ...
                                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 text-sm">
                                                        <div>
                                                            ...
                                                            <div className="text-gray-700">{localDate}</div>
                                                        </div>
                                                        <div className="sm:col-span-2">
                                                            <span className="text-[10px] font-bold text-gray-400 uppercase block mb-1">Description</span>
                                                            <div className="text-gray-700 whitespace-pre-wrap">{entry.description}</div>
                                                        </div>
                                                    </div>
                                                </div>

                                                <div className="flex items-center gap-4 pt-4 md:pt-0 border-t md:border-t-0 md:border-l border-gray-200 md:pl-6">
                                                    {entry.hasReceipt ? (
                                                        <a
                                                            href={`/api/user/other-expenses/${entry.id}/receipt`}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="flex flex-col items-center gap-1 group text-gray-500 hover:text-blue-600"
                                                        >
                                                        ...
                                                        <div className="flex flex-col items-center gap-1 text-gray-400">
                                                            <div className="w-10 h-10 rounded-xl flex items-center justify-center border border-dashed border-gray-300">
                                                    ...
                                                    <button
                                                        ...
                                                        className="px-3 py-2 text-xs font-bold text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all uppercase disabled:opacity-30"
                ...
                <div className="mt-12 text-center">
                    <Link href="/" className="inline-flex items-center gap-2 text-gray-500 hover:text-blue-600 font-medium">
                        ← Back to Home
                    </Link>
                </div>
            </div>
