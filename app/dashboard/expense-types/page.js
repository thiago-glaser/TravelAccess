        ...
        <div className="min-h-screen bg-gray-50 dark:bg-slate-800/50 text-slate-900 p-8">
            <div className="max-w-3xl mx-auto">
                <header className="mb-12">
                    ...
                    <p className="text-gray-500 dark:text-slate-400 mt-2">{t('expenses.manageTypesSubtitle') || 'Create categories for your other expenses'}</p>
                </header>
                ...
                <div className="grid gap-8">
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-xl">
                        ...
                        <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="flex flex-col gap-1">
                                <label className="text-sm text-gray-500 dark:text-slate-400 font-medium">Name</label>
                                <input
                                    type="text"
                                    value={name}
                                    onChange={(e) => setName(e.target.value)}
                                    className="px-4 py-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none w-full placeholder-gray-400"
                                    required
                                    placeholder="e.g. Tolls, Food, Lodging"
                                />
                            </div>
                            <div className="flex flex-col gap-1">
                                <label className="text-sm text-gray-500 dark:text-slate-400 font-medium">Description (optional)</label>
                                <input
                                    type="text"
                                    value={description}
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-xl overflow-hidden">
                        <div className="p-6 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center bg-gray-50 dark:bg-slate-800/50">
                            ...
                            <span className="text-xs bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-400 px-3 py-1 rounded-full border border-gray-200 dark:border-slate-700">{types.length} total</span>
                        </div>
                        <div className="divide-y divide-gray-100">
                            {loading ? (
                                <div className="p-10 text-center text-gray-500 dark:text-slate-400">Loading...</div>
                            ) : types.length === 0 ? (
                                <div className="p-10 text-center text-gray-500 dark:text-slate-400 font-medium italic">No expense types found. Add one above.</div>
                            ) : (
                                types.map((tItem) => (
                                    <div key={tItem.id} className="p-6 hover:bg-gray-50 dark:bg-slate-800/50 transition-colors flex justify-between items-center">
                                        <div>
                                            <div className="text-lg font-bold text-slate-800">{tItem.name}</div>
                                            <div className="text-sm text-gray-500 dark:text-slate-400">{tItem.description}</div>
                                        </div>
                                        <button
                                            onClick={() => handleRemove(tItem.id)}
                                            className="px-3 py-2 text-xs font-bold text-gray-500 dark:text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all uppercase tracking-widest disabled:opacity-30"
                                            disabled={submitting || userProfile?.isDemo}
                                        >
                                            Delete
                                        </button>
                ...
                <div className="mt-12 text-center">
                    <Link href="/dashboard/other-expenses" className="inline-flex items-center gap-2 text-gray-500 dark:text-slate-400 hover:text-blue-600 dark:text-blue-400 font-medium">
                        ← Back to Expenses
                    </Link>
                </div>
            </div>
