...
export async function GET(request, { params }) {
    ...
    try {
        ...
        // And we specify fetchInfo to get the BLOB properly
        const opts = {
            outFormat: oracledb.OUT_FORMAT_OBJECT,
            fetchInfo: { "RECEIPT_IMAGE": { type: oracledb.BUFFER } }
        };

        // We can't easily rely on our custom `query` wrapper to pass `fetchInfo` directly
        // unless it merges options. Let's assume `query` takes all options.
