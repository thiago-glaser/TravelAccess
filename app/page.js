...
export default function SessionsPage() {
    ...
    const [addFormData, setAddFormData] = useState({ deviceId: '', carId: '', startUtc: '', endUtc: '', sessionType: 'P' });
    ...
    const handleAddSession = async () => {
        ...
        if (!addFormData.deviceId || !addFormData.startUtc) {
            alert('Please provide at least a Device and Start Time.');
            return;
        }
        ...
        try {
            const response = await fetch('/api/sessions', {
                ...
                body: JSON.stringify({
                    ...
                    carId: addFormData.carId,
                    startUtc: new Date(addFormData.startUtc).toISOString(),
                    endUtc: addFormData.endUtc ? new Date(addFormData.endUtc).toISOString() : null,
                    sessionType: addFormData.sessionType
                })
            });
            const result = await response.json();
            if (result.success) {
                setIsAddModalOpen(false);
                setAddFormData({ deviceId: '', carId: '', startUtc: '', endUtc: '', sessionType: 'P' });
                fetchSessions(1);
            } else {
        ...
        <div className="min-h-screen bg-gray-50 pb-8">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                        ...
                        <button
                            onClick={() => {
                                const d = new Date();
                                const localIso = new Date(d.getTime() - d.getTimezoneOffset() * 60000).toISOString().slice(0, 16);
                                setAddFormData(prev => ({ ...prev, startUtc: localIso }));
                                setIsAddModalOpen(true);
                            }}
                            className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 flex items-center gap-2"
                        >
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {isAddModalOpen && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setIsAddModalOpen(false)}></div>
                    <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md animate-in zoom-in slide-in-from-bottom-4 duration-300">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h3 className="text-xl font-bold text-gray-900">{t('sessions.addSession') || 'Add Session'}</h3>
                        </div>

                        <div className="space-y-4 mb-6">
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1 ml-1">{t('sessions.table.device') || 'Device'}</label>
                                <select
                                    value={addFormData.deviceId}
                                    onChange={(e) => setAddFormData(prev => ({ ...prev, deviceId: e.target.value }))}
                                    className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"
                                >
                                    <option value="">{t('sessions.selectDevice') || 'Select Device'}</option>
                                    {devices.map(d => (
                                        <option key={d.DEVICE_ID} value={d.DEVICE_ID}>{d.DESCRIPTION || d.DEVICE_ID}</option>
                                    ))}
                                </select>
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1 ml-1">{t('sessions.table.car')}</label>
                                <select
                                    value={addFormData.carId}
                                    onChange={(e) => setAddFormData(prev => ({ ...prev, carId: e.target.value }))}
                                    className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"
                                >
                                    <option value="">{t('sessions.selectCar') || 'Select Car'}</option>
                                    {cars.map(car => (
                                        <option key={car.ID} value={car.ID}>{car.DESCRIPTION || car.LICENSE_PLATE || car.ID}</option>
                                    ))}
                                </select>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1 ml-1">{t('sessions.table.startTime')}</label>
                                    <input
                                        type="datetime-local"
                                        value={addFormData.startUtc}
                                        onChange={(e) => setAddFormData(prev => ({ ...prev, startUtc: e.target.value }))}
                                        className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1 ml-1">{t('sessions.table.endTime')}</label>
                                    <input
                                        type="datetime-local"
                                        value={addFormData.endUtc}
                                        onChange={(e) => setAddFormData(prev => ({ ...prev, endUtc: e.target.value }))}
                                        className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"
                                    />
                                </div>
                            </div>
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                className="flex-1 px-4 py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 active:scale-95"
                            >
                                {t('sessions.addSession') || 'Add Session'}
                            </button>
                        </div>
