...
export async function validateApiKey(key) {
    ...
    const sql = `
        ...
        WHERE ak.KEY_VALUE = :key AND ak.IS_ACTIVE = 1 AND (ak.IS_DELETED = 0 OR ak.IS_DELETED IS NULL)
    `;
    ...
    if (result.rows && result.rows.length > 0) {
        ...
        query('UPDATE API_KEYS SET LAST_USED = CURRENT_TIMESTAMP WHERE TRIM(ID) = TRIM(:id)', { id: apiKeyInfo.ID })
            .catch(err => console.error('Failed to update last_used:', err));

        return apiKeyInfo;
    }

    // Fallback: Check if it matches the ADMIN_API_KEY from .env
    if (process.env.ADMIN_API_KEY && key === process.env.ADMIN_API_KEY) {
        return {
            isAdmin: true,
            isDemo: false,
            USERNAME: 'system-admin',
            authType: 'env-admin-key'
        };
    }
