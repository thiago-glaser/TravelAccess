import { Location, SessionData } from '@/lib/models';
...
export async function runMergeLocationGeocodesJob() {
    try {
        ...
        const [locationsUpdated] = await Location.update(
            { locationGeocodeId: targetGeocodeId },
            { where: { locationGeocodeId: { [Op.in]: sourceGeocodeIds } } }
        );
