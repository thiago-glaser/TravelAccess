import { Location, LocationData, Device, sequelize } from '@/lib/models';
...
export async function runExtractPointsJob() {
    try {
        ...
        await Location.destroy({
            where: {
                timestampUtcStart: {
                    [Op.eq]: sequelize.col('TIMESTAMP_UTC_END')
                }
            }
        });

        // 2. Get distinct devices gracefully through the Devices table
        const rawDevices = await Device.findAll({
            attributes: ['deviceId'],
            order: [['deviceId', 'ASC']],
            raw: true
        });
        ...
        for (const deviceId of devices) {
            ...
            for (const loc of points) {
                ...
                const alt = Number(loc.altitude || 0);
                ...
                if (!openStartTs) {
                    ...
                    const dist = calculateDistance(openLat, openLon, lat, lon);
