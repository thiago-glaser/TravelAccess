import { DataTypes, Model } from 'sequelize';
import sequelize from '../sequelize.js';

class User extends Model {}

User.init({

    id: { type: DataTypes.CHAR(36), primaryKey: true, defaultValue: DataTypes.UUIDV4, field: 'ID' },
    username: { type: DataTypes.STRING(50), allowNull: false, unique: true, field: 'USERNAME' },
    passwordHash: { type: DataTypes.STRING(255), field: 'PASSWORD_HASH' },
    email: { type: DataTypes.STRING(100), field: 'EMAIL' },
    createdAt: { type: DataTypes.DATE, field: 'CREATED_AT' },
    isAdmin: { type: DataTypes.INTEGER, field: 'IS_ADMIN' },
    googleId: { type: DataTypes.STRING(255), field: 'GOOGLE_ID' },
    googleAvatarUrl: { type: DataTypes.STRING(500), field: 'GOOGLE_AVATAR_URL' },
    updatedAt: { type: DataTypes.DATE, field: 'UPDATED_AT' },
    isDeleted: { type: DataTypes.INTEGER, defaultValue: 0, field: 'IS_DELETED' },
    isDemo: { type: DataTypes.CHAR(1), defaultValue: 'N', field: 'IS_DEMO' },
    resetPasswordToken: { type: DataTypes.STRING(128), field: 'RESET_PASSWORD_TOKEN' },
    resetPasswordExpires: { type: DataTypes.DATE, field: 'RESET_PASSWORD_EXPIRES' },
    verificationToken: { type: DataTypes.STRING(128), field: 'VERIFICATION_TOKEN' },
    isVerified: { type: DataTypes.INTEGER, defaultValue: 0, field: 'IS_VERIFIED' },
    deletionToken: { type: DataTypes.STRING(128), field: 'DELETION_TOKEN' },
    deletionExpires: { type: DataTypes.DATE, field: 'DELETION_EXPIRES' }
}, {
    sequelize,
    modelName: 'User',
    tableName: 'USERS',
    timestamps: true,
    createdAt: 'createdAt',
    updatedAt: 'updatedAt'
});

export default User;
